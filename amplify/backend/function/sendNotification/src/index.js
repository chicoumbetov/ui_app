...
const NotificationQueries_1 = require("/opt/nodejs/src/NotificationQueries");
    ...
    if (event.userIds) {
        ...
        const map = userIds.map(async (userId) => {
            ...
            if (user) {
                ...
                let date = moment().add(-30, 'days').format('YYYY-MM-DDT00:00:00');
                if (user.privateProfile?.notificationLastSeenAt) {
                    const notificationLastSeenAtDate = DateUtils_1.default.parseToDateObj(user.privateProfile.notificationLastSeenAt);
                    const itemDate = DateUtils_1.default.parseToDateObj(date);
                    if (itemDate < notificationLastSeenAtDate) {
                        date = user.privateProfile?.notificationLastSeenAt;
                    }
                }
                const notificationCountQuery = await NotificationQueries_1.listNotificationsByUser(AppSyncClient, {
                    userId,
                    sortDirection: NotificationQueries_1.ModelSortDirection.DESC,
                    createdAt: {
                        ge: date,
                    },
                });
                const notificationCount = notificationCountQuery ? notificationCountQuery.length : 0;
        ...
        if (tokenList.length > 0) {
            const chunks = expo.chunkPushNotifications(tokenList.map(({ token, notificationId, count, countOnly, }) => {
                if (countOnly) {
                    return {
                        to: token,
                        badge: count,
